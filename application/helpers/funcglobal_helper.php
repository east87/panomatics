...
function formGenerate($controller,$ListLabel){
        ...
        foreach($ListLabel as $label){
            if ($label['type_id']==1) { 
                ...
                $content_image_thumbs = BASE_URL.str_replace('/admin/images','/admin/.thumbs/images',$label['content_text']);
...
function resultData($content,$controller){
        ...
        if ($value['label_view']==1) { 
           if ($value['type_id']==3) { 
                    $content_image_thumbs = BASE_URL.str_replace('/admin/images','/admin/.thumbs/images',$value['content_text']);
                    $content_image = BASE_URL.$value['content_text'];
                $html_output .='<td>';
                $html_output .='<a href="#" data-toggle="modal" data-target="#m_modal_'.$value['content_id'].'">';
                $html_output .='<img src="'.$content_image_thumbs.'" class="img-thumbnail" width="100">';
                $html_output .='</a>';
...
//           }
            ...
            $html_output .='<td>'.getOptions($value['content_text'],$value['label_id']).$value['content_text'];
             if($value['label_child'] && $value['content_text']==50 || $value['content_text']==51){
            $html_output .='<br>';
            $html_output .='<a href="'.BASE_URL_BACKEND.'/'.$controller.'/child/'.$value['label_id'].'/'.$value['content_id'].'">List Gallery</a>';                   
                }
            $html_output .='</td>';
...
if( ! function_exists('formGeneratex')){
function formGeneratex($controller,$ListLabel){
        $html_output='';
        foreach($ListLabel as $label){
            if ($label['type_id']==1) { 
                $html_output .= ' <div class="form-group m-form__group row '.$label['label_name'].'">
                           <label class="col-lg-2 col-form-label">
                            '.$controller.' '.$label['label_title'].':
                           </label>
                           <div class="col-lg-6">
                ...
                $html_output .= '<div class="form-group m-form__group row">
                                ...
                                <img src="'.$content_image.'" style="max-width:400px; padding:5px; border:solid 1px #ccc;">                                   
                                   ...
                                   <input type="text" name="'.$label['label_name'].'" readonly="'.$label['label_name'].'" id="'.$label['label_name'].'" class="form-control" value="'.$content_image.'">
                                   <p class="help-block" style="color:#00F;">'.$label['label_notif'].'</p>
                                   <div style="margin-right:10px;">
                                      <a class="btn btn-outline-brand m-btn m-btn--icon m-btn--icon-only m-btn--outline-2x" onClick="openKCFinder(&#39;'.$label['label_name'].'&#39;);" id="link-file" class="link"><i class="flaticon-attachment"></i></a>
                                      <a class="btn btn-outline-brand m-btn m-btn--icon m-btn--icon-only m-btn--outline-2x" onClick="reset_value(&#39;'.$label['label_name'].'&#39;);" id="link-file" class="link"><i class="fa fa-refresh"></i></a>
                                   </div>
                                </div>
                             </div>';
                ...
                $html_output .= '<div class="form-group m-form__group row '.$label['label_name'].'">
                                    ...
                                    <div class="col-lg-4 col-md-9 col-sm-12">
                                    <select  name="'.$label['label_name'].'" class="form-control m-input" id="'.$label['label_name'].'">
                                    <option value="">select '.$label['label_title'].'</option>';
...
if( ! function_exists('resultDatax')){
function resultDatax($content,$controller){
        $html_output='';
        foreach ($content as $value) {
        if ($value['label_view']==1) { 
           if ($value['type_id']==3) { 
                ...
                if($value['label_child']){
                $html_output .='<br>';
                $html_output .='<a class="sub-child" href="'.BASE_URL_BACKEND.'/'.$controller.'/child/'.$value['label_id'].'/'.$value['content_id'].'">List Gallery</a>';   
                }
                $html_output .='</td>';
                $html_output .='<div class="modal fade" id="m_modal_'.$value['content_id'].'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg" role="document">
            ...
            $html_output .='<td>'.getOptions($value['content_text'],$value['label_id']);
             if($value['label_child'] && $value['content_text'] =='50' || $value['content_text'] =='51'){
                $html_output .='<br>';
                $html_output .='<a class="sub-child" href="'.BASE_URL_BACKEND.'/'.$controller.'/child/'.$value['label_id'].'/'.$value['content_id'].'">List Gallery</a>';   
                }
...
if( ! function_exists('pagging')){
	function pagging($base_url, $total_rows, $per_page){
		...
		// Config link
		$config['full_tag_open'] = '<div class="m-datatable__pager m-datatable--paging-loaded clearfix"><ul class="m-datatable__pager-nav pagination">';
		$config['full_tag_close'] = '</ul></div>';
		
		$config['cur_tag_open'] = '<li class="active"><a class="m-datatable__pager-link m-datatable__pager-link-number m-datatable__pager-link--active" href="#">';
		$config['cur_tag_close'] = '</a></li>';
		
		$config['num_tag_open'] = '<li>';
		$config['num_tag_close'] = '</li>';
		
		$config['prev_link'] = '<a title="Prev" class="m-datatable__pager-link m-datatable__pager-link--prev"><i class="la la-angle-left"></i></a>';
		$config['prev_tag_open'] = '<li>';
		$config['prev_tag_close'] = '</li>';
		
		
		$config['next_tag_open'] = '<li> ';
                $config['next_link'] = '<a title="Next" class="m-datatable__pager-link m-datatable__pager-link--next"><i class="la la-angle-right"></i></a>';
