...
<head>
    ...
    <title>Statistic - Panomatics | Virtual Reality Solutions</title>
   ...
   <?php include 'include/vheadercontent.php';?>
    <div id="container-page">
        <div id="header-statistics">
            <h4>SOME INTERESING FACTS AND STAATISTICS ON THE ADDED VALUE AND BENEFITS VIRTUAL TOURS CAN OFFER YOU :</h4>
        </div>
        <div class="border-wrapper border-inside">
            <div class="border bg-load lazy-load" src-url="<?= IMAGES_BASE_URL; ?>/dot.jpg"></div>
        </div>
        <div id="statistic-thumbs-wrapper">
            <?php
                if($countContent > 0){
                $i=0;
                ...
                <div class="col-statistic col-33">
                <div class="statistic-thumb-box">
                    <div class="statistic-thumb-img information">
                        <div class="img-pano img-load lazy-load" src-url="<?=contentValue($content, 'images');?>" alt="<?= strtoupper(contentValue($content, 'title'));?>"></div>
                        <a href="#conten_statistic<?=$i?>">
                            <button class="button-statistic-item">LEARN MORE ABOUT <?= strtoupper(contentValue($content, 'title'));?></button> 
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div id="list-menu" class="info-statistics">
             <?php
                if($countContent > 0){
                $i=0;
                foreach($ListContent as $content){
            ...
            <div id="conten_statistic<?=$i?>" class="menu-content <?php if($i==1) {  echo 'current';} ?>">
                <div class="about-statistic">
                    <div id="about-statistic-title-wrapper">
                        <div class="logo-icon img-load lazy-load" src-url="<?= contentValue($content, 'icon');?>"></div>
                        <h4 id="about-statistic-title"><?= contentValue($content, 'title');?></h4>
                        <a id="close-statistic" data="<?=$i?>" onclick="hidContent(<?=$i?>)">X</a>
                    </div>
                    <div id="about-statistic-content">
    ...
    function hidContent(data) {
       
     var element = document.getElementById("conten_statistic"+data);
    element.className = element.className.replace(/\bcurrent\b/g, "");
    }
    </script>
</body>
