                     ...
                     <section id="unseen">
                         <form name="formAssignment" method="POST" action="" onsubmit="return false;">
                            <table class="m-datatable" id="html_table" width="100%">
                              ...
                              <tbody>
                                   <?php
                                    if(count($ListPrivateparty) > 0){
                                    $no=0;
                                    foreach($ListPrivateparty as $privateparty){
                                            $no++;
                                    ?>
                                    <tr>
                                      <td><?php echo $no;?></td>
                                      <td><?php echo $privateparty['privateparty_title'];?></td>
                                       <td>
                                                    <?php if($approve){ ?>
                                                            <?php if($privateparty['privateparty_active_status'] == 1) {?>
                                                                    <a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/privateparty/active/".$privateparty['privateparty_id'];?>"><i class="la la-check-circle"></i></a> 
                                                            <?php } else { ?>
                                                                    <a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/privateparty/active/".$privateparty['privateparty_id'];?>"><i class="la la-power-off"></i></a>
                                                            <?php } ?>
                                                    <?php } ?>
                                                    <?php if($edit){ ?>
                                                            <a class="btn-primary btn-sm" title="Click to Edit" href="<?php echo BASE_URL_BACKEND;?>/privateparty/edit/<?php echo $privateparty['privateparty_id'];?>"><i class="la la-edit"></i></a>
                                                    <?php } ?>
                                                    <?php if($delete){ ?>
                                                            <a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $privateparty['privateparty_title'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND;?>/privateparty/delete/<?php echo $privateparty['privateparty_id'];?>';}"><span class="la la-trash"></span></a>
                                                    <?php } ?>
                                            </td>
                                        </tr>
