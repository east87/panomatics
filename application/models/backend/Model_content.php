...
class Model_content extends CI_Model {
    function __construct()
    {
        ...
        function getChildContent($row_id,$label_id,$content_id, $module_id){
             
		 $sql	    = "SELECT distinct  a.row_id,  a.row_active_status, a.row_order, a.row_alias,"
                                    . " DATE_FORMAT( a.row_create_date, '%d-%m-%Y %H:%i:%s' ) as row_create_date,"
                                    . " a.row_create_by "                                    
                                    . " FROM tbl_row as a "
                                    . " inner join tbl_content as b on a.row_id = b.row_id"
                                    . " WHERE a.row_parent = $row_id and b.content_parent=$content_id";
                ...
                $data  = array();
                 foreach ($query as $row) {                    
                         $data[] = array(
                                ...
                                'content' =>$this->getContent2($row['row_id'],$label_id,$content_id,$module_id)
                            );                        
                      }
		return $data;
	}    
         ...
         function getListContent($module_id){
             $where = '';
		if($module_id != ''){
                ...
                $data  = array();
                 foreach ($query as $row) {                    
                         $data[] = array(
                                ...
                                'row_active_status' => $row['row_active_status'],
                                'row_create_date' => $row['row_create_date'],
                                'content' =>$this->getContent2($row['row_id'],0,0,$module_id)
                            );                        
                      }
		return $data;
	}
        
        function getContent($row_id,$label_id,$content_id)
    {
        $where = '';
        $label_parent = $label_id;
        $content_parent = $content_id;
        if ($row_id != '') {
            $where = "WHERE a.row_id = ".$row_id." and b.label_parent = $label_parent and a.content_parent=$content_parent and  b.label_view = 1 ";
        }
        $sql   = "SELECT a.content_id,b.label_id,a.content_text, b.type_id, b.label_name, b.label_view
                                FROM tbl_label as b 
...
//                       echo '-->';
	...
	function getContent2($row_id,$label_id,$content_id, $module_id)
    {
        $where = '';
        $label_parent = $label_id;
        $content_parent = $content_id;
    ...
    function getContentby2($row_id, $label_parent, $module_id)
    {
        ...
        $sql   = "SELECT a.content_id,b.label_id, a.content_text, b.type_id,
                            b.module_id, b.label_title,b.label_name,b.label_notif, b.label_view, b.type_id, b.label_view, 
                                ...
                                inner join tbl_label_type as c on b.type_id =c.type_id " 
                            . " $where " . " ORDER BY b.label_order ASC";
        $query = $this->db->query($sql)->result_array();
        $data  = array();
        if($query !='') {
	...
	function getContentby($module_id,$row_id)
    {
        ...
        $sql   = "SELECT a.content_id,b.label_id, a.content_text, b.type_id, b.module_id, b.label_title,b.label_name, b.type_id, b.label_view, b.label_notif, b.label_order, b.label_active_status, c.type_title "
               . "FROM tbl_label as b LEFT JOIN tbl_content as a on a.label_id =b.label_id and a.row_id = $row_id "
               . "inner join tbl_label_type as c on b.type_id =c.type_id "
               . " $where ORDER BY b.label_order ASC";
        $query = $this->db->query($sql)->result_array();
        $data  = array();
    ...
    function insertRow($module_id,$row_parent,$maximum)
...
function updateRowAlias($row_id,$alias)
...

	{
		...
		$sql	= "SELECT content_text FROM tbl_content WHERE content_parent = '".$content_parent."' "
                        . " and row_id = '".$row_id."' "
...

	{
		 $sql	= "UPDATE tbl_content SET 
                            ...
                            content_text='".$content_text."'"
                         . " WHERE label_id=$label_id and  row_id =".$row_id;	
		$query	= $this->db->query($sql);
		return $query;
         ...
         function updateOrderContent($id,$order)
	{
      ...
      function getRowParent($content_id){
       ...
       function getRow($row_id){
		...
		function getMaxRow($module_id){
            $data = array();
             $hasil =   $this->db->query("SELECT MAX(row_order) as maximum FROM tbl_row where module_id = $module_id ");	
			$data = $hasil->row_array();
                        return $data['maximum']; 
        }
         ...
         function getListContentRow($where, $order_limit, $module_id){
